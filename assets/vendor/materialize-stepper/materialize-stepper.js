...
$.fn.activateStepper = function() {
   $(this).each(function(){
      ...
      $stepper.wrap( '<form id="frmSteps" action="'+action+'" method="'+method+'">' );
...
// O3M
$.fn.disablePrevSteps = function (actualStep){
// Desactivar Steps anteriores
   if(!actualStep) actualStep = 1;
   for(var i=1; i<actualStep; i++){
     $(this).find("[data-step='" + i + "']").addClass('done');
   }
}
...
// Hace responsivo
   if($(window).width()<1200){
      $('.stepper').removeClass('horizontal');
   }else{
      if(!$('.stepper').hasClass('horizontal'))
         $('.stepper').addClass('horizontal');
   }
});
// O3M